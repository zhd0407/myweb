...
layui.define('fly', function(exports){
  ...
  $('body').on('click', '.question-admin', function(){
  ...
  var asyncRender = function(){
    ...
    if(jieAdmin[0] && layui.cache.user.uid != -1){
      fly.json('/collection/find/', {
        ...
        jieAdmin.append('<span class="layui-btn layui-btn-xs question-admin '+ (res.data.collection ? 'layui-btn-danger' : '') +'" type="collect" data-type="'+ (res.data.collection ? 'remove' : 'add') +'">'+ (res.data.collection ? '取消收藏' : '收藏') +'</span>');
      });
    }
  }();
        ...
        fly.json('/api/jieda-delete/', {
          ...
          if(res.status === 0){
            ...
            if(li.hasClass('jieda-daan')){
              $('.question-status').removeClass('jie-status-ok').text('求解中');
            }
          } else {
            layer.msg(res.msg);
          }
