    ...
    var tpl = [
      ...
      <li>\
        {{# if(d.rows[i].collection_time){ }}\
          <a class="question-title" href="/question/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
        ...
        {{# } else { }}\
          ...
          {{# if(d.rows[i].accept >= 0){ }}\
            <span class="question-status question-status-ok">已结</span>\
          {{# } else { }}\
            <span class="question-status">未结</span>\
          {{# } }}\
          {{# if(d.rows[i].status == -1){ }}\
            <span class="question-status">审核中</span>\
          {{# } }}\
          <a class="question-title" href="/question/{{d.rows[i].id}}/" target="_blank">{{= d.rows[i].title}}</a>\
          <i class="layui-hide-xs">{{ layui.util.timeAgo(d.rows[i].time, 1) }}</i>\
          {{# if(d.rows[i].accept == -1){ }}\
          <a class="mine-edit layui-hide-xs" href="/question/edit/{{d.rows[i].id}}" target="_blank">编辑</a>\
          {{# } }}\
      ...
      if(gather.minelog[type + '-page-' + curr]){
        ...
        if(type === 'collection'){
          ...
          fly.json('/api/'+ type +'/', {
            ...
            view(res);
            gather.minelog['mine-question-page-' + curr] = res;
            now || laypage.render({
              elem: 'LAY_page'
              ,count: res.count
              ,curr: curr
